...
const keyboard_key = {
	...
	template: '<div class="keyboard-key" v-on:click="focusKey"' +
			...
			'<select ref="select"' +
				...
				'v-on:input="updateKey($event.target.value, ' +
							'key_data)">' +
	...
	methods: {
		updateKey: function(new_type, new_data) {
			this.key_type = new_type;
			this.key_data = new_data;
			let validated_data = validateByType(this.key_data,
								this.key_type);
			this.key_data = validated_data;
			this.$refs.input.value = this.key_data;
			this.$emit('updatekey', this.x, this.y,
					this.key_type, this.key_data);
		},
		focusKey: function() {
...
socket.on('layermaps.c', layermaps_file_text => {
	console.log(layermaps_file_text);
...
socket.on('progress', report => {
	...
	if(report.op == "flash") {
		keyboard.server_error_message =
			"Reset to bootloader to continue...";
	}
...
fetch('layermaps.c')
	...
	.then(text => {
		if(!keyboard.server_connected) {
			...
			keyboard.current_layer = 0;
			keyboard.layers =
				getLayernamesFromFileData(keyboard.file_data);
		}
	});
...
function validateTypeNONE(data) {
	return "";
}
...
function createExportHeader() {
	let header = export_header_format.replace(/%%DATE%%/g,
...
function parseLayerMapsFile(text) {
	...
	for(constblock in consts) {
		let block = consts[constblock];
		let layername =
			block.replace(/[\s\n]/g,'').split(')')[0].slice(1);
		let rows = block.slice(block.indexOf('{')+1,
				       block.lastIndexOf('}'))
				.replace(/\s/g,'')
				.split('},');
		let layermap = [];
